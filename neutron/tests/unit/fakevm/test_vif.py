...
class TestFakeVMVif(base.BaseTestCase):
    def setUp(self):
        ...
        self.mod_vif = importutils.import_module('neutron.debug.fakevm.vif')

        self.mock_cfg = mock.patch('neutron.debug.fakevm.vif.cfg').start()
        ...
        self.importutils = mock.patch(
            'neutron.debug.fakevm.vif.importutils').start()
        self.importutils.import_class = self.import_class

    def mock_vif(self):
        self.conf = mock.Mock()
        self.conf.libvirt_vif_driver = 'vifdriver'
        return self.mod_vif.NeutronFakeVMVifWrapper(self.conf)
    ...
    def test_main(self):
        ...
        with nested(
            mock.patch.object(self.mod_vif, 'NeutronFakeVMVifWrapper'),
