...
from neutron.debug.fakevm import shell
...
class TestFakeVMShell(base.BaseTestCase):
    def setUp(self):
        ...
        self.mock_cfg = mock.patch('neutron.debug.fakevm.shell.cfg').start()
        self.mock_cfg.CONF = self.mock_conf
        self.mock_config = mock.patch(
            'neutron.debug.fakevm.shell.config').start()
        self.mock_rpc = mock.patch(
            'neutron.debug.fakevm.shell.fakevm_rpc').start()

    def test_init(self):
        shell.NeutronFakeVMShell('2.0')

    def test_build_option_parser(self):
        fvshell = shell.NeutronFakeVMShell('2.0')

        parser = mock.Mock()
        with mock.patch(
            'neutronclient.shell.NeutronShell.build_option_parser',
    ...
    def test_initialize_app(self):
        ...
        with mock.patch('neutronclient.shell.NeutronShell.initialize_app') as (
                mock_initialize_app):
