...
class FakeVMCommand(NeutronCommand):
    ...
    def _get_network(self, network_id):
        client = self.get_neutron_client()
    ...
    def _create_port(self, network, instance_id):
        ...
        port = self.get_neutron_client().create_port(body)['port']
        port['instance_id'] = instance_id
        return port

    def _delete_port(self, vif_uuid):
        self.get_neutron_client().delete_port(vif_uuid)

    def _show_port(self, vif_uuid):
        port = self.get_neutron_client().show_port(vif_uuid)['port']
