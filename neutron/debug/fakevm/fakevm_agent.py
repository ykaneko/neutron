...
class NeutronFakeVMAgent(object):
    OPTS = [
        cfg.StrOpt('host',
    ...
    def __init__(self, conf):
        super(NeutronFakeVMAgent, self).__init__()
...
def main():
    ...
    cfg.CONF.register_cli_opts(NeutronFakeVMAgent.OPTS, 'FAKEVM')
    config.register_agent_state_opts_helper(cfg.CONF)
    config.register_root_helper(cfg.CONF)
    cfg.CONF(project='neutron')
    config.setup_logging(cfg.CONF)

    agent = NeutronFakeVMAgent(cfg.CONF)
    agent.wait_rpc()
