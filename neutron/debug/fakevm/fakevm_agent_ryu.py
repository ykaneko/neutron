...
from neutron.debug.fakevm import fakevm_agent_plugin_base
from neutron.extensions import portbindings
from neutron.plugins.ryu.agent import ryu_neutron_agent
from neutron.plugins.ryu.common import config  # noqa


class NeutronFakeVMAgentRyu(
        fakevm_agent_plugin_base.NeutronFakeVMAgentPluginBase):
    OPTS = [
    ...
    def __init__(self):
        super(NeutronFakeVMAgentRyu, self).__init__()
        self.conf = None
        self.root_helper = None
        self.int_br = None

    def init(self, conf):
        super(NeutronFakeVMAgentRyu, self).init(conf)
    ...
    def _get_tunnel_name(self):
        # host name can have similar leading part. e.g. guest-1, guest-2
        # use the trailing part in order to avoid name clash
        return 'qft' + self.host[-self.DEV_NAME_LEN + 3:]

    def _init_tunnel(self):
        self._cleanup_tunnel()

        dev_name = self._get_tunnel_name()
        tunnel_ip = ryu_neutron_agent._get_tunnel_ip()
        ...
        device.addr.add(4, tunnel_ip, '+')
        # ip link set $tunnel_interface up
        device.link.set_up()
        #self._execute(['ip', 'route', 'add', tunnel_ip, 'dev', dev_name])
...
cfg.CONF.register_opts(NeutronFakeVMAgentRyu.OPTS, 'FAKEVM')
