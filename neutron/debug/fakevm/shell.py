...
from neutron.debug.fakevm import rpc as fakevm_rpc


_COMMAND_PREFIX = 'neutron.debug.fakevm.commands'
...
class NeutronFakeVMShell(NeutronShell):
    def __init__(self, api_version):
        super(NeutronFakeVMShell, self).__init__(api_version)
        for k, v in COMMANDS[api_version].items():
            self.command_manager.add_command(k, v)

    def build_option_parser(self, description, version):
        parser = super(NeutronFakeVMShell, self).build_option_parser(
            description, version)
        parser.add_argument(
            '--config-file',
            default=env('NEUTRON_FAKEVM_CONFIG_FILE'),
            help=_('Config file for fakevm shell '))
        return parser

    def initialize_app(self, argv):
        super(NeutronFakeVMShell, self).initialize_app(argv)
        if not self.options.config_file:
            raise exc.CommandError(
                _('You must provide a config file for bridge -'
                  ' either --config-file or env[NEUTRON_FAKEVM_CONFIG_FILE]'))
...
def main(argv=None):
    ...
    return NeutronFakeVMShell(NEUTRON_API_VERSION).run(argv or sys.argv[1:])
